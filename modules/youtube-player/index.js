...
import { OptionsContext } from "@options"; // @ts-ignore
...
const Player = ({
  ...
  playerSetting,
  videoIds,
  isForwordTimeDuration = true,
  forwordTimeDurationInSeconds = 5,
  isBackwordTimeDuration = true,
  backwordTimeDurationInSeconds = 5
}) => {
  ...
  const handleNext = () => {
    if (youTubeRef.current) {
      youTubeRef.current.getCurrentTime().then(currentTime => {
  ...
  return <ScrollView>
      <YouTube ref={youTubeRef} apiKey={youtubeApiKey || options.YOUTUBE_API_KEY} videoIds={videoIds || options.VIDEOS_IDS} play={playerData.isPlaying} loop={playerData.isLooping} fullscreen={playerData.fullscreen} controls={1} onError={e => setPlayerData({ ...playerData,
      error: e.error
    })} onReady={() => setPlayerData({ ...playerData,
      isReady: true
    })} onChangeState={e => setPlayerData({ ...playerData,
      status: e.state
    })} onChangeQuality={e => setPlayerData({ ...playerData,
      quality: e.quality
    })} onChangeFullscreen={e => setPlayerData({ ...playerData,
      fullscreen: e.isFullscreen
    })} onProgress={e => setPlayerData({ ...playerData,
      currentTime: e.currentTime
    })} style={[styles.player, _styles.LqirQYln]} />
      ...
      {(isForwordTimeDuration || isBackwordTimeDuration) && <View style={styles.buttonGroup}>
          ...
          {isBackwordTimeDuration && <TouchableOpacity style={styles.button} onPress={handleNext}>
              <Icon name="back" size={30} color="#900" style={_styles.JSnNZaEh} />
